...
cur = db.cursor()
cur.execute("SELECT id,mobile FROM ads WHERE fetched=2 and country in(%s) ORDER BY rand() limit %s" % (FormatList(countries), str(many)),
            countries)
...
for m in mobiles:
    ...
    try:
        ...
        mobile = m['mobile']
        text = [
            'Log on to Shoutit.com to join the future of marketplaces. A location-based market that allows you \'shout\' your needs directly to your local community.'
            ,
