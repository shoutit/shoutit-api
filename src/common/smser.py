...
for user in users:
	...
	try:
		...
		text = _('There is potential interest in your ad \'%(shout_title)s\' on Shoutit.\nVisit Region\'s first FREE social marketplace %(link)s to get started') % {'shout_title' : title, 'link' : link}
		...
		if user.Country == 'US':
			...
			sms_url = 'https://www.smsglobal.com/http-api.php?&action=sendsms&user=syrexme&password=25600696&from=Shoutit.com&to=%s&text=%s' %(mobile, quoted_text)
