...
def CancelPaymentForService(user, service_code, remote_transaction_data,
                            remote_transaction_identifier):
    pass
...
def PayForDeal(user, deal, amount, remote_transaction_data, remote_transaction_identifier):
    import shoutit.controllers.deal_controller as deal_controller
    ...
    transaction, old_status = UpdateOrCreateTransaction(remote_transaction_identifier,
                                                        remote_transaction_data, 'Paid')
    if old_status != 'Paid':
...
def PayForService(user, service_code, amount, remote_transaction_data,
                  remote_transaction_identifier):
    if not amount:
        amount = 1
    else:
...
def GetPaypalFormForDeal(deal, user, amount=1):
    return PayPalEncryptedPaymentsForm(initial={
        ...
        'amount': '%.2f' % ConvertCurrency(deal.item.price * amount, deal.item.currency.code,
                                           'USD'),
...
def GetCPSPFormForDeal(deal, user, amount=1):
    cpsp_dict = {
        ...
        'ORDERID': 'D_%s_U_%s_x_%d_%s' % (deal.pk, user.pk, amount, str(time.time())),
        # todo: check
    ...
    h = hashlib.sha512()
    h.update(''.join(
        [k + '=' + cpsp_dict[k] + settings.CPSP_PASS_PHRASE for k in sorted(cpsp_dict.keys())]))
...
def CheckTransaction(pdt):
    ...
    if item_type == 'Deal':
        ...
        deal = deal[0]
        gross = '%.2f' % ConvertCurrency(deal.item.price * int(amount), deal.item.currency.code,
                                         'USD')
