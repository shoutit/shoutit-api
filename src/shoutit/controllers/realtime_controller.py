...
from shoutit.api.renderers import render_notification
...
def SendNotification(notification, username, count=0):
    ...
    from shoutit.controllers.message_controller import UnReadConversationsCount
    from shoutit.controllers.notifications_controller import get_user_notifications_without_messages_count
    try:
        from common.tagged_cache import TaggedCache
        apns_tokens = TaggedCache.get('apns|%s' % username)
        apns_count = 0
