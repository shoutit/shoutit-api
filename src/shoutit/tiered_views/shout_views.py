...
def inbound_email(request):
    ...
    if request.method == 'GET':
        ...
        message = send_message(conversation=None, user=from_user, to_users=[from_user,to_user], about=shout, text=text)
    ...
    try:
        if shout['type'] == 'request':
            shout = shout_controller.post_request(
                ...
                tags=tags, images=shout['images'], shouter=user, is_sss=True, exp_days=settings.MAX_EXPIRY_DAYS_SSS, category=category
            )
