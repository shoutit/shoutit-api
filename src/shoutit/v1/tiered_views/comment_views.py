...
@non_cached_view(methods=['POST'],
                 login_required=True,
                 json_renderer=lambda request, result, post_id: comment_on_post_json_renderer(
                     request, result),
                 validator=lambda request, post_id: comment_on_post_validator(request, post_id,
                                                                              CommentForm),
...
@non_cached_view(methods=['POST'],
                 ...
                 validator=lambda request, comment_id: delete_comment_validator(request,
                                                                                comment_id),
)
...
@non_cached_view(methods=['GET'],
                 json_renderer=lambda request, result, post_id: post_comments_json_renderer(request,
                                                                                            result),
                 validator=lambda request, post_id: object_exists_validator(get_post, True, _(
                     'post dose not exist.'), post_id),
)
