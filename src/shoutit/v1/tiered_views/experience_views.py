...
from shoutit.controllers.experience_controller import (GetExperience, GetBusinessThumbsCount,
                                                       GetExperiences, GetExperiencesCount,
                                                       PostExperience, EditExperience,
                                                       GetUsersSharedExperience, ShareExperience)
...
from shoutit.v1.tiered_views.renderers import (page_html, experiences_stream_json,
                                               post_experience_json_renderer, json_renderer,
                                               user_json_renderer)
from shoutit.tiered_views.validators import (experience_validator, share_experience_validator,
                                             edit_experience_validator,
                                             object_exists_validator, experience_view_validator)
from shoutit.tiered_views.tiers import ResponseResult, non_cached_view


@non_cached_view(methods=['GET'], validator=experience_view_validator,
                 html_renderer=lambda request, result, *args: page_html(request, result,
                                                                        'experience.html',
                                                                        result.data['page_title'] if
                                                                        result.data[
                                                                            'page_title'] else '',
                                                                        result.data['page_desc'] if
                                                                        result.data[
                                                                            'page_desc'] else ''))
def view_experience(request, exp_id):
    ...
    if experience:
        result.data['recent_experiences'] = GetExperiences(user=request.user,
                                                           about_business=result.data[
                                                               'experience'].AboutBusiness,
        ...
        result.data['page_title'] = '%s Experience with %s' % (
            'Bad' if experience.state == 0 else 'Good', experience.AboutBusiness.name)
...
@non_cached_view(methods=['GET'],
                 json_renderer=lambda request, result, *args: experiences_stream_json(request,
                                                                                      result))
...
def get_business_initials(username):
    ...
    init = {'name': business.name, 'category': cat,
            'location': str(business.latitude) + ', ' + str(business.longitude),
            'country': business.country, 'city': business.city, 'address': business.address,
            'username': username}
    return init


@csrf_exempt
@non_cached_view(methods=['POST'], login_required=True,
                 permissions_required=[PERMISSION_POST_EXPERIENCE],
                 json_renderer=lambda request, result, username: post_experience_json_renderer(
                     request, result),
                 validator=lambda request, *args, **kwargs: experience_validator(request,
                                                                                 initial=get_business_initials(
                                                                                     args and args[
                                                                                         0] or (
...
def post_exp(request, username=None):
    ...
    if username is None or username == '':
        ...
        latlng = 'location' in business_form.cleaned_data and business_form.cleaned_data[
            'location'] or ''
        ...
        business = CreateTinyBusinessProfile(
            ...
            'category' in business_form.cleaned_data and business_form.cleaned_data[
                'category'] or None,
            lat, lng,
            'country' in business_form.cleaned_data and business_form.cleaned_data[
                'country'] or None,
            'city' in business_form.cleaned_data and business_form.cleaned_data['city'] or None,
            'address' in business_form.cleaned_data and business_form.cleaned_data[
                'address'] or None,
            'source' in business_form.cleaned_data and business_form.cleaned_data['source'] or None,
            'source_id' in business_form.cleaned_data and business_form.cleaned_data[
                'source_id'] or None)
    else:
        business = GetBusiness(username)

    result.data['experience'] = PostExperience(request.user, int(EXPERIENCE_UP) if int(
        form.cleaned_data['state']) else int(
...
@non_cached_view(methods=['POST'], login_required=True,
                 ...
                 json_renderer=lambda request, result, exp_id: json_renderer(request, result,
                                                                             success_message=_(
...
@non_cached_view(methods=['POST'],
                 json_renderer=lambda request, result, exp_id: post_experience_json_renderer(
                     request, result, message=_(
...
def edit_experience(request, exp_id):
    ...
    form = ExperienceForm(request.POST)
    form.is_valid()
    result.data['experience'] = EditExperience(exp_id, EXPERIENCE_UP if int(
        form.cleaned_data['state']) else EXPERIENCE_DOWN,
...
@non_cached_view(methods=['GET'],
                 ...
                 validator=lambda request, exp_id: object_exists_validator(get_post, True, _(
                     'Experience dose not exist.'), exp_id),
                 )
