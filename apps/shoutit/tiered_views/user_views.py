...
from apps.shoutit.controllers import user_controller, facebook_controller
...
def activate_modal(request, token):
	...
	if user_controller.GetProfile(user):
		user_controller.LoginWithoutPassword(request, user)
...
@non_cached_view(json_renderer=lambda request, result, *args: activate_renderer_json(request, result),
	...
	validator=lambda request, *args, **kwargs: form_validator(request,
		...
		initial=request.user.Profile.isSSS and {'mobile': request.user.Profile.Mobile,
												'username': request.user.username} or {'email': request.user.email}),
	...
	if request.method == 'POST':
		if user.Profile.isSSS:
			...
			user_controller.CompleteSignUpSSS(request, form.cleaned_data['firstname'], form.cleaned_data['lastname'],
				...
				form.cleaned_data['email'], bool(int(form.cleaned_data['sex'])), form.cleaned_data['birthdate'])
...
@non_cached_view(methods=['POST'],
	validator=lambda request, *args, **kwargs: form_validator(request, RecoverForm,
		message=_('Username or email you entered does not exist.')),
...
@non_cached_view(json_renderer=resend_activation_json,
...
@non_cached_view(methods=['GET', 'POST'],
	...
	json_renderer=lambda request, result, username: json_renderer(request,
		result,
		_('You are now listening to %(name)s\'s shouts.') % {
...
@non_cached_view(html_renderer=lambda request, result: page_html(request, result, 'signup.html', 'Sign Up'),
	json_renderer=lambda request, result: json_renderer(request,
		result,
		success_message=_('Congratulations! You are now a member of the Shout community.')),
	api_renderer=operation_api,
	methods=['GET', 'POST'],
...
@cached_view(level=CACHE_LEVEL_USER,
	...
	html_renderer=lambda request, result, username, *args: object_page_html(request, result, 'user_profile.html',
		result.data.has_key('user_profile') and result.data['user_profile'].username or ''),
...
def user_stream(request, username, page_num=None):
	...
	result.data['shouts'] = stream_controller.GetStreamShouts(user_profile.Stream, DEFAULT_PAGE_SIZE * (page_num - 1), DEFAULT_PAGE_SIZE * page_num)
...
@cached_view(level=CACHE_LEVEL_USER,
	...
	methods=['GET'],
	api_renderer=user_api,
	html_renderer=lambda request, result, username, *args: object_page_html(request, result, isinstance(user_controller.GetUser(username), UserProfile) and 'user_profile.html' or 'business_profile.html',
		result.data.has_key('profile') and result.data['profile'].name() or '', result.data.has_key('profile') and result.data['profile'].Bio or ''),
	validator=user_profile_validator)
...
#	result.data['shouts_count'] = profile.User.Posts.GetValidShouts(types=[POST_TYPE_BUY,POST_TYPE_SELL]).count()
	...
	if isinstance(profile, BusinessProfile):
		...
		result.data['recent_experiences'] = experience_controller.GetExperiences(user=request.user, about_business=profile,start_index=0,end_index=5)
#		result.data['recent_items'] = gallery_controller.GetBusinessGalleryItems(profile,0,5)
	...
	result.data['fb_og_url'] = 'http%s://%s/user/%s/' % ('s' if settings.IS_SITE_SECURE else '',settings.SHOUT_IT_DOMAIN, profile.User.username)
	return result
...
@non_cached_view(json_renderer = lambda request, result: json_renderer(request, result, _('Thank you, we will send your friends your invitation.')))
def send_invitiations(request):
	result = ResponseResult()
	emails = []
