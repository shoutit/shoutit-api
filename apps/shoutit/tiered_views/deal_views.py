...
from apps.shoutit.controllers import deal_controller, user_controller, business_controller
from apps.shoutit.controllers.payment_controller import CONCURRENT_DEALS_SERVICE
	...
	if result.valid:
		...
		if False: #concurrent_deals:
			...
			if not service_buy or service_buy[0]['buys_count'] - service_buy[0]['used_count'] < 1:
				return ValidationResult(False, {}, [RESPONSE_RESULT_ERROR_BAD_REQUEST], [('error', _('You can\'t have concurrent deals, you must buy that service.'))])
	return result
...
@cached_view(
	...
	methods = ['GET'],
	validator = lambda request, deal_id: object_exists_validator(deal_controller.GetDeal, _('Deal does not exist.'), Base62ToInt(deal_id)),
	json_renderer = deal_renderer_json,
	html_renderer = lambda request, result, deal_id: page_html(request, result, 'deal.html'),
)
def view_deal(request, deal_id):
	deal = deal_controller.GetDeal(Base62ToInt(deal_id))
...
def view_deals(request):
	result = ResponseResult()
	result.data['deals'] = deal_controller.GetOpenDeals(request.user.is_authenticated() and request.user or None, country_code = request.session.has_key('user_country') and request.session['user_country'] or '', province_code = request.session.has_key('user_city') and request.session['user_city'] or '')
	return result

@non_cached_view(html_renderer=lambda request, result: page_html(request, result, 'paypal.html'))
def paypal(request):
	result = ResponseResult()
	result.data['form'] = apps.shoutit.controllers.payment_controller.GetPaypalFormForDeal(Deal.objects.get(pk = 17407), User.objects.get(username = 'mpcabd'), 3)
	result.data['sform'] = apps.shoutit.controllers.payment_controller.GetPaypalFormForSubscription(User.objects.get(username = 'mrabooode'))
	result.data['cpsp_dict'] = apps.shoutit.controllers.payment_controller.GetCPSPFormForDeal(Deal.objects.get(pk = 17407), User.objects.get(username = 'mpcabd'), 3)
	return result
	...
	if request.POST.has_key('STATUS') and request.POST.has_key('orderID'):
		...
		if match:
			...
			if request.POST['STATUS'] in ['5', '9']:
				if item_type == 'D':
					apps.shoutit.controllers.payment_controller.PayForDeal(user_id, item_id, amount, transaction_data, transaction_identifier)
				elif item_id == 'SERVICE':
					apps.shoutit.controllers.payment_controller.PayForService(user_id, item_id, amount, transaction_data, transaction_identifier)
				else:
					pass
			elif request.POST['STATUS'] in ['6', '7', '8']:
				if item_type == 'D':
					apps.shoutit.controllers.payment_controller.CancelPaymentForDeal(user_id, item_id, transaction_data, transaction_identifier)
				elif item_id == 'SERVICE':
					apps.shoutit.controllers.payment_controller.CancelPaymentForService(user_id, item_id, transaction_data, transaction_identifier)
				else:
					pass
...
def deals_stream(request,business_name, page_num=None):
	...
	result.data['deals'] = deal_controller.GetOpenDeals(request.user.is_authenticated() and request.user or None,business,start_index,end_index) 
	return result
