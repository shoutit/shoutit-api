...
#def create_stores(user):
#	for i in range(10):
#		store = StoreController.CreateStore(user.User, 'Store_%d' % i, 'this is Store_%d' %i, 'www.store_%d.com' %i,
#											'+971000000%d' % i, None, 20.4234 + i, 50.32432 + i)
...
#	def setUp(self):
#		create_users()
#		self.user = UserController.GetUser('User_0')
#		create_stores(self.user)
#
#	def test_GetStores(self):
#		stores = StoreController.GetStores(None)
#		self.assertEqual(len(stores), 10, "Retrieved stores count is not correct")
#
#	def test_GetStore(self):
#		store = StoreController.GetStore(1)
#		self.assertNotEqual(store, None, "Could not get Store_0")
#		if store is not None:
#			self.assertEqual(store.Name, 'Store_0', 'Retrieved Store_0 is not the correct one')
#
#	def test_GetStoreByName(self):
#		store = StoreController.GetStoreByName('Store_0')
#		self.assertNotEqual(store, None, 'Could not get Store_0 by name')
#		if store is not None:
#			self.assertEqual(store.Name, 'Store_0', 'Retrieved Store_0 by name is not the correct one ')
#
#	def test_StoreFollowers(self):
#		store = StoreController.GetStoreByName('Store_0')
#		UserController.FollowStream(None, self.user, store.Stream)
#		store_follower_names = StoreController.StoreFollowers(store.id)
#		res = self.user.User.username in store_follower_names
#		self.assertEqual(res, True, 'Could not retrieve Store_0 follower' + self.user.User.username)
...
#			store = StoreController.GetStoreByName('Store_0')
			...
			shout_sell = ShoutController.ShoutSell(None, 'Shout_sell_%d' % i, 'this is shout__sell_%d' % i,
												   ...
												   self.user.User, 'AE', 'Dubai', '', 'aed')
#			shout_exp = ShoutController.ShoutExperience(None, 'this is shout_exp_%d' % i, 20.4534747 + i, 50.78233 + i, ['tag_1', 'tag_2', 'tag_3', 'tag_4', 'tag_5', 'tag_6'],
#														self.user.User, store, 'AE', 'Dubai', '')
...
#			self.assertNotEqual(shout_exp, None, 'Could not create shout_exp_%d' % i)
...
##			self.assertNotEqual(shout_exp, None, 'Could not create shout_exp_%d' % i)
#
#	def test_GetUserRecommendedShoutStream(self):
#		user = UserController.GetUser('User_1')
#		UserController.FollowStream(None, user, self.user.Stream)
#		for i in range(1,32):
#			flag = RankTypeFlag()
#			flag.value = i
#			shouts = StreamController.GetUserRecommendedShoutStream(user.User, flag ,myLat= 20.2434, myLong=50.636)
#			self.assertNotEqual(len(shouts) , 0, 'Could not retrieve any shout from User_0')
#
#	def test_GetStreamShouts(self):
#		shouts = StreamController.GetStreamShouts(self.user.Stream)
#		self.assertEqual(len(shouts), self.user.Stream.Shouts.count(), "Could not retrieve User_0's stream shouts")
