...
def GetUsersSharedExperience(exp_id):
	experience_sharedExperiences = SharedExperience.objects.filter(Experience__pk = exp_id).select_related('Experience','OwnerUser','OwnerUser__Profile')
...
def GetExperience(user, exp_id, detailed=False):
	experience = Experience.objects.GetValidExperiences().filter(pk = exp_id).select_related('AboutBusiness','AboutBusiness__Profile','OwnerUser','OwnerUser__Profile').order_by('-DatePublished')
	if experience:
		experience = experience[0]
		experience.detailed = detailed
		if detailed:
			sharedExperiences = SharedExperience.objects.filter(Experience__pk = exp_id).select_related('Experience','OwnerUser','OwnerUser__Profile')
...
def GetExperiences(user, owner_user = None, about_business = None,start_index=None, end_index=None, detailed = False, city = None):
	...
	experiences = Experience.objects.GetValidExperiences().filter(pk__in = experiences_post_ids).select_related('AboutBusiness','AboutBusiness__Profile','OwnerUser','OwnerUser__Profile').order_by('-DatePublished')[start_index:end_index]
	sharedExperiences = SharedExperience.objects.filter(Experience__pk__in = experiences_post_ids).select_related('Experience','OwnerUser','OwnerUser__Profile')
...
def getDetailedExperience(user,experience,sharedExperiences,comments):
	experience.usersSharedExperience = [sharedExperience.OwnerUser for sharedExperience in sharedExperiences if sharedExperience.Experience.pk == experience.pk]
