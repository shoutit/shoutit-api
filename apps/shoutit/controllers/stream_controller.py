...
def MaxFollowings(ids, country_code, province_code, filters):
	shouts = Trade.objects.GetValidTrades(country_code=country_code, province_code=province_code).filter(**filters).select_related('Streams').filter(Streams__pk__in = ids).values('StreamsCode')
	mutuals = [len(set(int(f) for f in shout['StreamsCode'].split(',') if len(shout['StreamsCode'].strip()) > 0) & set(ids)) for shout in shouts]
...
def GetRankedShoutsIDs(user, rank_type_flag, country_code = '', province_code = '' , lat=0.0, long = 0.0, start_index=None, end_index=None, filter_types = [], filter_query = None, filter_tags = []):
...
#		initializing variables
	...
	user_followings_ids = []
	if user is not None:
		user_followings_ids = [x['id'] for x in Stream.objects.filter(followship__follower__pk = user_controller.GetProfile(user).id).values('id')]
	where_custom_clause = []
	additional_selects = []
	group_by_string = ''
	...
	if filter_tags is not None and len(filter_tags) > 0:
		if filters.has_key('Tags__pk__in'):
	...
	if int(rank_type_flag & FOLLOW_RANK_TYPE):
		if user is not None:
			...
			shout_qs.query.join((None, Shout._meta.db_table, None, None))
			shout_qs.query.join((
			Shout._meta.db_table,
			Shout.Streams.field.m2m_db_table(),
			Shout._meta.pk.column,
	...
	try:
		...
		pass
	return [
	(row[1], row[0]) for row in cursor.fetchall() if row and len(row)
	]


def __GetStreamOfShoutsWithTags(rank_flag , user = None, lat = None, long= None, country_code = '', province_code = '', show_expired=False, *args , **filters):
	if not country_code:
		country_code = ''
	if not province_code:
		province_code = ''
	end_index = None
	if filters.has_key('end_index'):
		end_index = filters['end_index']
		del filters['end_index']
	start_index = None
	if filters.has_key('start_index'):
	...
	shout_qs = Trade.objects.GetValidTrades(country_code=country_code, province_code=province_code, get_expired=show_expired).select_related('Item', 'Item__Currency', 'OwnerUser', 'OwnerUser__Profile', 'Tags').filter(*args , **filters)

	#		Calculating the Distance axis:
	if int(rank_flag & DISTANCE_RANK_TYPE):
		# A temproray solution for max distance:
		points = list(Trade.objects.GetValidTrades(country_code=country_code, province_code = province_code).filter(**filters).values('Latitude', 'Longitude'))
		if lat is not None and long is not None:
			max_distance = MaxDistance(points, lat, long)
			if max_distance > 0.0:
	...
	if int(rank_flag & FOLLOW_RANK_TYPE):
		if user is not None:
			...
			shout_sq = shout_qs.filter(Streams__pk__in = ids)
			shout_qs.query.join((None, Shout._meta.db_table, None, None))
			shout_qs.query.join((
			Shout._meta.db_table,
			Shout.Streams.field.m2m_db_table(),
	...
	if extra_order_bys <> '':
		...
		shout_qs = shout_qs.distinct(True).order_by('-DatePublished')[start_index:end_index]
...
def GetShoutRecommendedShoutStream(base_shout, type, start_index = None, end_index = None, exclude_shouter = True):
	...
	extra_order_bys = ''
	points = list(Trade.objects.GetValidTrades(country_code=base_shout.CountryCode, province_code=base_shout.ProvinceCode).filter(**filters).values('Latitude', 'Longitude'))
	...
	if shouts:
		...
		tags_with_shout_id = []
	images = StoredImage.objects.filter(Q(Shout__pk__in = [shout.pk for shout in shouts if shout.Type == POST_TYPE_EXPERIENCE]) | Q(Item__pk__in = [shout.Item.pk for shout in shouts if shout.Type != POST_TYPE_EXPERIENCE])).order_by('Image')
...
#		RETURNS: array of shout objects
	...
	if shouts:
		...
		tags.query.join((None, Tag._meta.db_table, None, None))
		connection = (Tag._meta.db_table, Trade.Tags.field.m2m_db_table(), Tag._meta.pk.column, Trade.Tags.field.m2m_reverse_name())
		tags.query.join(connection, promote=True)
		tags = tags.extra(where=['shout_id IN (%s)' % ','.join([str(shout.pk) for shout in shouts])])
		tags_with_shout_id = list(tags)
	else:
...
def GetStreamShouts(stream,  start_index=None, end_index=None, show_expired=False, country = '', province = ''):
	return __GetStreamOfShoutsWithTags(TIME_RANK_TYPE ,user = None, lat = None, long = None, country_code = country, province_code = province, IsMuted__exact = False, Streams__pk__in = [stream.pk], start_index=start_index, end_index=end_index, show_expired=show_expired)
