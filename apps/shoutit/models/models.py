...
class PostManager(models.Manager):
	...
	def filter_shout_expired(self,query_set):
		...
		query_set = query_set.filter(
			...
			| (
				  (Q(Type = POST_TYPE_BUY)|Q(Type = POST_TYPE_SELL))
				& (Q(shout__ExpiryDate__isnull = True, DatePublished__range = (begin, today)) | Q(shout__ExpiryDate__isnull = False, shout__ExpiryDate__gte = today))
			)
		)
		return query_set
...
class Tag(models.Model):
	...
	Image = models.URLField(max_length=1024, null=True, default='/static/new_design/img/shout_tag.png')
	DateCreated = models.DateTimeField(auto_now_add=True)
