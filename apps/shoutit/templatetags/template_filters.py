...
def thumbnail(url, size):
	return utils.get_size_url(url, size)
...
def secure_url(url):
	...
	if not url.startswith('https'):
		...
		if cdn_re.match(url):
			return utils.get_https_cdn(url)
...
def set_trans_attr(field, attr):
	text = attr.split(':', 1)[1]
	import translating_variables as tv
...
@register.simple_tag
def name2url(name):
	return utils.ToSeoFriendly(name)

@register.simple_tag
def shout_link(post):
	return utils.ShoutLink(post)