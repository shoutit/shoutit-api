...
BEGIN
	return (select max(subq.rank) from(
		...
		from "ShoutWebsite_shout" as shout INNER JOIN "ShoutWebsite_shout_Streams" AS shout_streams ON (shout.id = shout_streams.shout_id)
		INNER JOIN "ShoutWebsite_stream" AS stream ON
		(shout_streams.stream_id = stream.id) INNER JOIN "ShoutWebsite_followship" AS followship ON
